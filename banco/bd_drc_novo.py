...
con = pymysql.connect(host="localhost", user="gabic", passwd="1234", db="drc")
con.select_db('drc')

banco_int= pd.read_csv('/home/gabic9814/git/DRC/banco/Dados2010-2014DesfechoFasesETC2.csv')
cfr= pd.read_csv('/home/gabic9814/git/DRC/banco/cfr2.csv')
tfg=  pd.read_csv('/home/gabic9814/git/DRC/banco/v.csv')
...
for i in range(len(banco_int)): 
    ...
    if str(banco_int.to_numpy()[i][33]) <= '17.0':
        classe_imc = '1'
    #abaixo do peso    
    elif str(banco_int.to_numpy()[i][33]) >= '17.0' and str(banco_int.to_numpy()[i][33]) <= '18.49':
        classe_imc = '2'
    #peso normal    
    elif str(banco_int.to_numpy()[i][33]) >= '18.5' and str(banco_int.to_numpy()[i][33]) <= '24.99':
        classe_imc = '3'  
    #acima do peso    
    elif str(banco_int.to_numpy()[i][33]) >= '25.0' and str(banco_int.to_numpy()[i][33]) <= '29.99':
        classe_imc = '4'
    #Obesidade I    
    elif str(banco_int.to_numpy()[i][33]) >= '30.0' and str(banco_int.to_numpy()[i][33]) <= '34.99':
        classe_imc = '5'
    #Obesidade II
    elif str(banco_int.to_numpy()[i][33]) >= '35.0' and str(banco_int.to_numpy()[i][33]) <= '39.99':
        classe_imc = '6'
    #Obesidade III
    elif str(banco_int.to_numpy()[i][33]) >= '40.0':
        classe_imc = '7'
    ...
    ##Inserindo cidade
    sql1="INSERT INTO cidade(nome) VALUES ('"+banco_int.to_numpy()[i][12]+"')"
    cursor.execute(sql1)
    cursor.execute('SELECT last_insert_id() INTO @cidade')
    cursor.fetchone()
    
    ##Inserindo ubs
    sql2="INSERT INTO ubs(nome) VALUES('"+banco_int.to_numpy()[i][13]+"')"
    cursor.execute(sql2)
    ...
    z=list(banco_int.to_numpy()[i][221:229])
    ...
    r=list(banco_int.to_numpy()[i][213:221])
    ...
    m=list(banco_int.to_numpy()[i][229:237])
    cursor.execute(sql6,list(m))
    cursor.execute('SELECT last_insert_id() INTO @dm')
    cursor.fetchone()      
#    
    ##Inserindo tfg
    sql7="INSERT INTO calculo_tfg(creatinina_1_2011, creatinina_2_2011,	creatinina_1_2012,creatinina_2_2012,	creatinina_1_2013,creatinina_2_2013,	creatinina_1_2014,creatinina_2_2014,tfg_1_2011, EQ ,tfg_2_2011, EQ1,tfg_1_2012, EQ2 ,tfg_2_2012, EQ3,tfg_1_2013, EQ4,tfg_2_2013, EQ5,tfg_1_2014, EQ6,tfg_2_2014,EQ7) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    t=list(tfg.to_numpy()[i][238:262])
    ...
    trs=list(banco_int.to_numpy()[i][94:98])
    trs.extend(banco_int.to_numpy()[i][90:94])
...
#    
    ...
    med=list(banco_int.to_numpy()[i][23:33])
...
#    
    ...
    trat=list(banco_int.to_numpy()[i][66:70])
    trat.extend(banco_int.to_numpy()[i][198:202])
    ...
    afr=list(banco_int.to_numpy()[i][42:46])
    afr.extend(banco_int.to_numpy()[i][142:146])
    afr.extend(banco_int.to_numpy()[i][158:166])
    afr.extend(banco_int.to_numpy()[i][186:190])
    afr.extend(banco_int.to_numpy()[i][194:198])
    afr.extend(banco_int.to_numpy()[i][46:50]) #tshi_inicial	,dt_tshi_inicial,tshi_final,	dt_tshi_final,
    afr.extend(banco_int.to_numpy()[i][106:110]) #potassio_inicial,dt_potassio_inicial,potassio_final,dt_potassio_final
    afr.extend(banco_int.to_numpy()[i][206:210]) #ureia_inicial,dt_ureia_inicial,ureia_final,dt_ureia_final
    afr.extend(banco_int.to_numpy()[i][114:118]) #ureia24h_inicial,dt_ureia24h_inicial,ureia24h_final,dt_ureia24h_final
    afr.extend(banco_int.to_numpy()[i][130:142]) #fosforo_inicial,dt_fosforo_inicial,fosforo_final,dt_fosforo_final,pth_intacto_inicial	,dt_pth_intacto_inicial,	pth_intacto_final,dt_pth_intacto_final,vitaminaD_inicial,dt_vitaminaD_inicial,vitaminaD_final,dt_vitaminaD_final
    afr.extend(banco_int.to_numpy()[i][190:194]) #fosfatase_alc_inicial,dt_fosfatase_alc_inicial,fosfatase_alc_final,dt_fosfatase_alc_final,
    afr.extend(banco_int.to_numpy()[i][202:206]) #ck_inicial,dt_ck_inicial,ck_final,dt_ck_final,
    afr.extend(banco_int.to_numpy()[i][166:178]) #ecoae_inicial,dt_ecoae_inicial,ecoae_final,dt_ecoae_final,ecoao_inicial,	dt_ecoao_inicial	,ecoao_final,dt_ecoao_final,	ecosiv_inicial,dt_ecosiv_inicial	,ecosiv_final,dt_ecosiv_final
    ...
    cfr_list=list(cfr.to_numpy()[i][:72])   
    cursor.execute(sql12,list(cfr_list))
    cursor.execute('SELECT last_insert_id() INTO @cfr')
    cursor.fetchone()     
    
    #Inserindo dados_consulta
    estagioI = banco_int.to_numpy()[i][253] # estagio inicial TFG
    estI = estagioI[8:10] 
    estagioF = banco_int.to_numpy()[i][254] # estagio inicial TFG
    estF = estagioF[8:10]
    ##calculando o estágio das taxas de albuminúria
    #inicial
    taxaI_alb = banco_int.to_numpy()[i][142]
    estI_alb = albuminTaxa(taxaI_alb,estI,i)
    #final
    taxaF_alb = banco_int.to_numpy()[i][144]
    estF_alb = albuminTaxa(taxaF_alb,estF,i)
    
    sql16="INSERT INTO dados_consulta(data_inicial,	data_final,	tempoAcomp,	total_consultas_drc,	total_consultas_dm,	total_consultas_has,	desfecho	,desfechotxt,	id_cidade,	id_afr,	id_ubs,	id_has,	id_drc,	id_dm,	id_cfr,	id_exame_fisico,	id_calculo_tfg,	id_preparacao_trs,	id_medicamento,	id_aval_tratamento,estagioI,estagioF,estagioI_alb,estagioF_alb) VALUES('"+str(banco_int.to_numpy()[i][18])+"','"+str(banco_int.to_numpy()[i][19])+"','"+str(banco_int.to_numpy()[i][35])+"','"+str(banco_int.to_numpy()[i][21])+"','"+str(banco_int.to_numpy()[i][20])+"','"+str(banco_int.to_numpy()[i][22])+"','"+str(banco_int.to_numpy()[i][211])+"','"+str(banco_int.to_numpy()[i][212])+"',@cidade,@afr,@ubs,@has,@drc,@dm,@cfr,@exame_fisico,@tfg,@trs,@medicamento,@tratamento,'"+str(estI)+"','"+str(estF)+"','"+str(estI_alb)+"','"+str(estF_alb)+"')"   
...
#
    ##Inserindo raca
    sql13 = "INSERT INTO raca(nome) VALUES('" + banco_int.to_numpy()[i][4] + "')"
    cursor.execute(sql13)
    cursor.execute('SELECT last_insert_id() INTO @raca')
    cursor.fetchone()
    
    ##Inserindo Instrucao
    sql14 = "INSERT INTO instrucao(nome) VALUES('"+banco_int.to_numpy()[i][8]+"')"

    ...
    for dataset in datasets:#[:1]:
        ...
        for tk, tn in enumerate(dataset['target_names']):
            ...
            distributions = dict(C=uniform(loc=0, scale=1e4),
                                 ...
                                 epsilon=uniform(loc=0, scale=10),
                               )
            
            clf = RandomizedSearchCV(estimator=SVR(kernel='rbf'), 
            ...
            pl.figure()
            pl.plot(y_test, y_test, '-', y_test, y_pred,'o')
            r2=r2_score(y_test, y_pred)
            r=stats.pearsonr(y_test, y_pred)[0]
            rmse=RMSE(y_test, y_pred)                
            mape=MAPE(y_test, y_pred)   
            pl.ylabel(dataset_name)
            pl.title('SVR'+': (Testing)\nR$^2$='+str('%1.3f' % r2)+'\t RMSE='+str('%1.3f' % rmse)
                          +'\t MAPE ='+str('%1.3f' % mape)
                      )
            ...
            # todo: 
            #(1) salvar os parametros em arquivo 
            #(2) plotar a distribuição dos parametros
            #(3) discutir os resultados das métricas e a distribuição dos parametros
            #(4) colocar tudo no paper CIIS
#%%----------------------------------------------------------------------------   
